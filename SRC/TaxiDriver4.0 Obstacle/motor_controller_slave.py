...
def motor_controller_process(motor_speed, motor_direction, encoder_value, shutdown_flag):
    """
    Motor controller process for multiprocessing communication
    
    Args:
        motor_speed (multiprocessing.Value): Shared speed value (0-100)
        motor_direction (multiprocessing.Value): Shared direction value (1=forward, -1=reverse, 0=stop)
        encoder_value (multiprocessing.Value): Shared encoder count value
        shutdown_flag (multiprocessing.Value): Flag to signal process shutdown
    """
    controller = None
    try:
        print("Motor controller process starting...")
        ...
        while not shutdown_flag.value:
            try:
                # Read shared values
                current_speed = motor_speed.value
                current_direction = motor_direction.value
                
                # Update motor if values changed
                if current_speed != last_speed or current_direction != last_direction:
                    if current_direction == 0 or current_speed == 0:
                        controller.stop()
                    else:
                        controller.set_speed(current_speed, current_direction)
                    
                    last_speed = current_speed
                    last_direction = current_direction
                
                # Update encoder value
                encoder_value.value = controller.get_encoder_count()
                
                # Small delay to prevent excessive CPU usage
                time.sleep(0.01)  # 10ms update rate
                
            except Exception as e:
                print(f"Motor controller process error: {e}")
                time.sleep(0.1)
        
        print("Motor controller process shutting down...")
        
    except KeyboardInterrupt:
        print("\nMotor controller process interrupted")
    except Exception as e:
        print(f"Motor controller process error: {e}")
    finally:
        if controller:
