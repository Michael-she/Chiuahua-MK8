...
OBSTACLE_PROXIMITY = 15.0    # Max distance between points to be in the same cluster
OBSTACLE_MIN_POINTS = 3      # Min number of points to form a valid obstacle cluster
OBSTACLE_MAX_SIZE = 30.0     # <<< NEW: Max radius for a cluster to be considered an obstacle
...
class Robot:
    ...
    def estimate_walls(self):
        ...
        self.detect_obstacles(remaining_points)
    ...
    def fit_line_with_pca(self, points):
        ...
        line_start, line_end = mean + t_min * direction_vector, mean + t_max * direction_vector
        return (line_start.tolist(), line_end.tolist())

    # <<< MODIFIED METHOD: detect_obstacles >>>
    def detect_obstacles(self, points):
        """Finds clusters of points not associated with a wall."""
        self.detected_obstacles = []
        ...
        while unvisited_points:
            ...
            while head < len(queue):
                ...
                neighbors = []
                for other_point in list(unvisited_points):
                    if math.hypot(current_point[0] - other_point[0], current_point[1] - other_point[1]) < OBSTACLE_PROXIMITY:
            ...
            # If the cluster has enough points, check its size
            if len(current_cluster) >= OBSTACLE_MIN_POINTS:
                cluster_np = np.array(current_cluster)
                center = np.mean(cluster_np, axis=0)
                distances = [math.hypot(p[0] - center[0], p[1] - center[1]) for p in current_cluster]
                radius = max(distances) + 5
                
                # <<< MODIFIED SECTION START >>>
                # Only add if the obstacle is smaller than the maximum size
                if radius <= OBSTACLE_MAX_SIZE:
                    self.detected_obstacles.append({'center': center.tolist(), 'radius': radius})
...
def main():
    ...
    pygame.display.set_caption("Wall and Obstacle Estimation")
    ...
    while running:
        ...
        for obstacle in robot.detected_obstacles:
            pygame.draw.circle(screen, COLOR_OBSTACLE_DETECTED, obstacle['center'], obstacle['radius'], 2)
