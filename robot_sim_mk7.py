...
COLOR_PARKING_WALL = (128, 0, 128) # Purple for parking walls
COLOR_PARKING_WALL_DETECTED = (0, 255, 0) # Green when detected
COLOR_PARKING_ZONE = (0, 255, 0, 50) # Translucent green for detected parking area
...
PARKING_WALL_LENGTH = 20
PARKING_WALL_SEPARATION = 60
PARKING_DETECTION_THRESHOLD = 15.0  # Distance threshold for detecting parking walls
PARKING_CONFIDENCE_THRESHOLD = 3.0  # Confidence needed to confirm parking zone
...
def generate_obstacles():
    # First, generate the parking zone
    parking_zone = generate_parking_zone()
    parking_segment = parking_zone['segment']
    ...
    # Filter out obstacle slots in the parking segment that are between halfway mark and outside wall
    filtered_obstacle_slots = {}
    for segment_name, slots in obstacle_slots.items():
        filtered_slots = []
        segment_num = {"top": 1, "right": 2, "bottom": 3, "left": 4}[segment_name]
        
        if segment_num == parking_segment:
            # Remove slots between halfway mark and outside wall
            for slot in slots:
                if not is_slot_in_parking_restricted_area(slot, parking_segment):
                    filtered_slots.append(slot)
        else:
            filtered_slots = slots
        
        filtered_obstacle_slots[segment_name] = filtered_slots
    
    generated_obstacles, obstacle_walls = [], []
    for _, slots in filtered_obstacle_slots.items():
        if not slots:  # Skip if no valid slots in this segment
            continue
        num_to_spawn = random.choice([1, 2 ,2])
        chosen_slots = []
        if num_to_spawn == 1: 
            chosen_slots.append(random.choice(slots))
        elif num_to_spawn == 2 and len(slots) >= 2: 
            if len(slots) >= 6:
                pair = random.choice([(0, 2), (3, 5), (0, 5), (2, 3)])
                if pair[0] < len(slots) and pair[1] < len(slots):
                    chosen_slots.extend([slots[pair[0]], slots[pair[1]]])
                else:
                    chosen_slots = random.sample(slots, min(2, len(slots)))
            else:
                chosen_slots = random.sample(slots, min(2, len(slots)))
    ...
    # Add parking zone walls as both physical walls and visual obstacles
    if parking_zone:
        wall1, wall2 = parking_zone['wall1'], parking_zone['wall2']
        
        # Add parking walls as physical walls for LiDAR detection
        obstacle_walls.extend([wall1, wall2])
        
        # Create visual obstacles for parking walls (for camera simulation)
        for wall in [wall1, wall2]:
            if parking_zone['orientation'] == 0:  # Horizontal parking (vertical walls)
                # Create vertical wall obstacle
                wall_x = wall[0]
                wall_y_start = min(wall[1], wall[3])
                wall_y_end = max(wall[1], wall[3])
                wall_length = wall_y_end - wall_y_start
                
                # Create multiple small obstacles along the wall
                num_segments = max(1, wall_length // 4)  # 4-pixel segments
                for i in range(int(num_segments)):
                    segment_y = wall_y_start + (i * wall_length / num_segments)
                    segment_rect = pygame.Rect(wall_x - 2, segment_y, 4, wall_length / num_segments)
                    generated_obstacles.append({'rect': segment_rect, 'color': COLOR_PARKING_WALL})
                    
            else:  # Vertical parking (horizontal walls)
                # Create horizontal wall obstacle
                wall_y = wall[1]
                wall_x_start = min(wall[0], wall[2])
                wall_x_end = max(wall[0], wall[2])
                wall_length = wall_x_end - wall_x_start
                
                # Create multiple small obstacles along the wall
                num_segments = max(1, wall_length // 4)  # 4-pixel segments
                for i in range(int(num_segments)):
                    segment_x = wall_x_start + (i * wall_length / num_segments)
                    segment_rect = pygame.Rect(segment_x, wall_y - 2, wall_length / num_segments, 4)
                    generated_obstacles.append({'rect': segment_rect, 'color': COLOR_PARKING_WALL})
    
    return generated_obstacles, obstacle_walls, parking_zone
...
class Robot:
    def __init__(self, x, y):
        ...
        self.detected_parking_walls = []
        self.parking_zone_confidence = 0.0

    def detect_parking_zone(self, parking_zone, obstacles_for_drawing):
        """
        Detect parking zone using LiDAR and camera data.
        LiDAR will always detect the physical walls, but camera is needed to confirm purple color.
        The robot detects the parking zone by:
        1. Finding two parallel walls with correct spacing in LiDAR data
        2. Confirming they are purple using camera
        """
        if parking_zone['detected']:
            return  # Already detected
            
        wall1, wall2 = parking_zone['wall1'], parking_zone['wall2']
        
        # Check if we can find these walls in our estimated walls from LiDAR
        parking_walls_found = []
        
        for estimated_wall in self.estimated_walls:
            wall_start = estimated_wall[0]
            wall_end = estimated_wall[1]
            
            # Check if this estimated wall matches either parking wall
            for parking_wall in [wall1, wall2]:
                parking_start = (parking_wall[0], parking_wall[1])
                parking_end = (parking_wall[2], parking_wall[3])
                
                # Check if the estimated wall overlaps significantly with the parking wall
                if self.walls_match(wall_start, wall_end, parking_start, parking_end):
                    parking_walls_found.append(parking_wall)
                    break
        
        # Check camera for purple color confirmation
        purple_confirmed = False
        if len(parking_walls_found) >= 1:  # At least one wall detected by LiDAR
            purple_confirmed = self.check_purple_walls_with_camera([wall1, wall2], obstacles_for_drawing)
        
        # Update confidence based on detection
        if len(parking_walls_found) == 2 and purple_confirmed:
            # Both walls detected by LiDAR and purple confirmed by camera
            self.parking_zone_confidence += 2.0
        elif len(parking_walls_found) >= 1 and purple_confirmed:
            # At least one wall detected and purple confirmed
            self.parking_zone_confidence += 1.0
        elif len(parking_walls_found) >= 1:
            # Wall(s) detected but no purple confirmation
            self.parking_zone_confidence += 0.3
        else:
            # No walls detected, decay confidence
            self.parking_zone_confidence = max(0, self.parking_zone_confidence - 0.1)
        
        # Store detected walls
        self.detected_parking_walls = parking_walls_found
        
        # Confirm parking zone if confidence is high enough
        if self.parking_zone_confidence >= PARKING_CONFIDENCE_THRESHOLD:
            parking_zone['detected'] = True
            print(f"Parking zone detected in segment {parking_zone['segment']}!")
    
    def walls_match(self, wall1_start, wall1_end, wall2_start, wall2_end, threshold=15.0):
        """
        Check if two walls match within a threshold distance.
        """
        # Calculate distances between corresponding points
        start_dist = math.hypot(wall1_start[0] - wall2_start[0], wall1_start[1] - wall2_start[1])
        end_dist = math.hypot(wall1_end[0] - wall2_end[0], wall1_end[1] - wall2_end[1])
        
        # Also check reversed direction
        start_dist_rev = math.hypot(wall1_start[0] - wall2_end[0], wall1_start[1] - wall2_end[1])
        end_dist_rev = math.hypot(wall1_end[0] - wall2_start[0], wall1_end[1] - wall2_start[1])
        
        # Return true if either direction matches
        return (start_dist < threshold and end_dist < threshold) or \
               (start_dist_rev < threshold and end_dist_rev < threshold)
    
    def check_purple_walls_with_camera(self, walls, obstacles_for_drawing):
        """
        Simulate camera detection of purple walls.
        Check if there are purple obstacles near the wall positions.
        """
        purple_walls_found = 0
        
        for wall in walls:
            wall_start = (wall[0], wall[1])
            wall_end = (wall[2], wall[3])
            
            # Sample points along the wall to check for purple color
            num_samples = 5
            purple_found_for_this_wall = False
            
            for i in range(num_samples):
                t = i / max(1, num_samples - 1)
                sample_x = wall_start[0] + t * (wall_end[0] - wall_start[0])
                sample_y = wall_start[1] + t * (wall_end[1] - wall_start[1])
                
                # Check if there's a purple-colored obstacle near this point
                found_purple, color = get_obstacle_at_coord(int(sample_x), int(sample_y), obstacles_for_drawing)
                if found_purple and color == COLOR_PARKING_WALL:
                    purple_found_for_this_wall = True
                    break
            
            if purple_found_for_this_wall:
                purple_walls_found += 1
        
        # Return true if we found purple color on at least one wall
        return purple_walls_found >= 1
...
def generate_parking_zone():
    """
    Generate a parking zone in one of the four segments.
    Returns a dictionary with parking zone information.
    """
    # Define possible parking positions for each segment
    parking_positions = {
        1: [  # Top segment
            ((COURSE_TOP_LEFT + 150, COURSE_TOP_LEFT + 25), 0),  # Horizontal parking
            ((COURSE_TOP_LEFT + 300, COURSE_TOP_LEFT + 25), 0),
            ((COURSE_TOP_LEFT + 450, COURSE_TOP_LEFT + 25), 0),
        ],
        2: [  # Right segment  
            ((COURSE_TOP_LEFT + COURSE_WIDTH - 25, COURSE_TOP_LEFT + 150), 90),  # Vertical parking
            ((COURSE_TOP_LEFT + COURSE_WIDTH - 25, COURSE_TOP_LEFT + 300), 90),
            ((COURSE_TOP_LEFT + COURSE_WIDTH - 25, COURSE_TOP_LEFT + 450), 90),
        ],
        3: [  # Bottom segment
            ((COURSE_TOP_LEFT + 150, COURSE_TOP_LEFT + COURSE_WIDTH - 25), 0),  # Horizontal parking
            ((COURSE_TOP_LEFT + 300, COURSE_TOP_LEFT + COURSE_WIDTH - 25), 0),
            ((COURSE_TOP_LEFT + 450, COURSE_TOP_LEFT + COURSE_WIDTH - 25), 0),
        ],
        4: [  # Left segment
            ((COURSE_TOP_LEFT + 25, COURSE_TOP_LEFT + 150), 90),  # Vertical parking
            ((COURSE_TOP_LEFT + 25, COURSE_TOP_LEFT + 300), 90),
            ((COURSE_TOP_LEFT + 25, COURSE_TOP_LEFT + 450), 90),
        ]
    }
    
    # Choose a random segment and position
    segment = random.choice([1, 2, 3, 4])
    position, orientation = random.choice(parking_positions[segment])
    
    # Generate the two parallel walls
    if orientation == 0:  # Horizontal parking (walls are vertical)
        wall1 = (position[0] - PARKING_WALL_SEPARATION // 2, position[1], 
                 position[0] - PARKING_WALL_SEPARATION // 2, position[1] + PARKING_WALL_LENGTH)
        wall2 = (position[0] + PARKING_WALL_SEPARATION // 2, position[1], 
                 position[0] + PARKING_WALL_SEPARATION // 2, position[1] + PARKING_WALL_LENGTH)
    else:  # Vertical parking (walls are horizontal)
        wall1 = (position[0], position[1] - PARKING_WALL_SEPARATION // 2, 
                 position[0] + PARKING_WALL_LENGTH, position[1] - PARKING_WALL_SEPARATION // 2)
        wall2 = (position[0], position[1] + PARKING_WALL_SEPARATION // 2, 
                 position[0] + PARKING_WALL_LENGTH, position[1] + PARKING_WALL_SEPARATION // 2)
...
def is_slot_in_parking_restricted_area(slot_pos, parking_segment):
    """
    Check if an obstacle slot is in the restricted area of the parking segment.
    The restricted area is between the halfway mark and the outside wall.
    """
    x, y = slot_pos
    
    if parking_segment == 1:  # Top segment
        # Restricted area is in the outer half (y < halfway point)
        halfway_y = COURSE_TOP_LEFT + (INNER_TL[1] - COURSE_TOP_LEFT) // 2
        return y < halfway_y
    elif parking_segment == 2:  # Right segment
        # Restricted area is in the outer half (x > halfway point)
        halfway_x = INNER_TR[0] + (COURSE_TOP_LEFT + COURSE_WIDTH - INNER_TR[0]) // 2
        return x > halfway_x
    elif parking_segment == 3:  # Bottom segment
        # Restricted area is in the outer half (y > halfway point)
        halfway_y = INNER_BR[1] + (COURSE_TOP_LEFT + COURSE_WIDTH - INNER_BR[1]) // 2
        return y > halfway_y
    elif parking_segment == 4:  # Left segment
        # Restricted area is in the outer half (x < halfway point)
        halfway_x = COURSE_TOP_LEFT + (INNER_TL[0] - COURSE_TOP_LEFT) // 2
        return x < halfway_x
    
    return False

def draw_parking_zone(screen, parking_zone):
    """
    Draw the parking zone walls and detected area.
    """
    if not parking_zone:
        return
        
    wall1, wall2 = parking_zone['wall1'], parking_zone['wall2']
    detected = parking_zone['detected']
    
    # Choose wall color based on detection status
    wall_color = COLOR_PARKING_WALL_DETECTED if detected else COLOR_PARKING_WALL
    
    # Draw parking walls
    pygame.draw.line(screen, wall_color, (wall1[0], wall1[1]), (wall1[2], wall1[3]), 4)
    pygame.draw.line(screen, wall_color, (wall2[0], wall2[1]), (wall2[2], wall2[3]), 4)
    
    # Draw translucent parking area if detected
    if detected:
        # Calculate parking area rectangle
        if parking_zone['orientation'] == 0:  # Horizontal parking (vertical walls)
            # Walls are vertical, parking area is between them
            left_x = min(wall1[0], wall2[0])
            right_x = max(wall1[0], wall2[0])
            top_y = min(wall1[1], wall1[3], wall2[1], wall2[3])
            bottom_y = max(wall1[1], wall1[3], wall2[1], wall2[3])
            
            parking_rect = pygame.Rect(left_x, top_y, right_x - left_x, bottom_y - top_y)
        else:  # Vertical parking (horizontal walls)
            # Walls are horizontal, parking area is between them
            left_x = min(wall1[0], wall1[2], wall2[0], wall2[2])
            right_x = max(wall1[0], wall1[2], wall2[0], wall2[2])
            top_y = min(wall1[1], wall2[1])
            bottom_y = max(wall1[1], wall2[1])
            
            parking_rect = pygame.Rect(left_x, top_y, right_x - left_x, bottom_y - top_y)
        
        # Create translucent surface for parking area
        parking_surface = pygame.Surface((parking_rect.width, parking_rect.height), pygame.SRCALPHA)
        parking_surface.fill(COLOR_PARKING_ZONE)
        screen.blit(parking_surface, (parking_rect.x, parking_rect.y))
...
def distance_between_points(p1, p2):
    ...
    Args:
        p1, p2: Tuples representing the (x, y) coordinates of the two points.
...
def main():
    ...
    obstacles_for_drawing, obstacle_walls, parking_zone = generate_obstacles()
    ...
    while running:
        ...
        # Detect parking zone
        robot.detect_parking_zone(parking_zone, obstacles_for_drawing)
        ...
        for obs in obstacles_for_drawing: 
            # Change color for detected parking walls
            display_color = obs['color']
            if obs['color'] == COLOR_PARKING_WALL and parking_zone and parking_zone['detected']:
                display_color = COLOR_PARKING_WALL_DETECTED
            pygame.draw.rect(screen, display_color, obs['rect']); pygame.draw.rect(screen, COLOR_WALL, obs['rect'], 1)
        ...
        # Draw parking zone
        draw_parking_zone(screen, parking_zone)
        ...
        # Display parking zone status
        if parking_zone:
            segment_names = {1: "Top", 2: "Right", 3: "Bottom", 4: "Left"}
            parking_segment_name = segment_names.get(parking_zone['segment'], 'Unknown')
            if parking_zone['detected']:
                parking_text = f"Parking Zone: DETECTED in {parking_segment_name} segment"
                parking_color = COLOR_PARKING_WALL_DETECTED
            else:
                parking_text = f"Parking Zone: {parking_segment_name} segment (confidence: {robot.parking_zone_confidence:.1f})"
                parking_color = COLOR_TEXT
            screen.blit(small_font.render(parking_text, True, parking_color), (10, 165))
        ...
        if current_time - fps_update_timer > 500:
            if fps_history:
        ...
        fps_y_pos = SCREEN_HEIGHT - 25
        screen.blit(small_font.render(fps_text, True, COLOR_TEXT), (10, fps_y_pos))
